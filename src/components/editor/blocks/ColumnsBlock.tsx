...
import { PreviewContext, BlockProps, ChildBlockWrapper } from "./shared";
...
function ColumnDropZone({ zoneId, blocks, label, flexBasis }: { zoneId: string; blocks: Block[]; label: string; flexBasis: string }) {
    ...
    return (
        <div
            ref={setNodeRef}
            style={{ flex: isStacked ? "none" : `0 0 ${flexBasis}`, width: isStacked ? "100%" : undefined, minHeight: isPreview && blocks.length === 0 ? 0 : 80, border: isPreview ? "none" : `2px dashed ${isOver ? "#6366f1" : "#e2e8f0"}`, borderRadius: 6, background: !isPreview && isOver ? "rgba(99,102,241, 0.04)" : "transparent", transition: "all 0.15s", overflow: "hidden" }}
        >
            {blocks.length > 0 ? (
