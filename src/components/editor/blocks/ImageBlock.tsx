...
export function ImageBlock({ block }: BlockProps) {
    ...
    if (!src) {
        return (
            <div id={(p.sectionId as string) || `block-${block.id}`} style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 8, height: 200, width: "100%", background: "var(--surface)", border: "2px dashed var(--border)", color: "var(--text-subtle)" }}>
    ...
    return (
        <div id={(p.sectionId as string) || `block-${block.id}`} style={{ padding: "8px 16px", width: "100%" }}>
