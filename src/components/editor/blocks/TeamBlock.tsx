...
export function TeamBlock({ block }: BlockProps) {
    ...
    const getCardStyle = (): React.CSSProperties => {
        ...
        if (cardStyle === "filled") return { ...base, background: p.cardBg ? cardBg : (isDark ? "#ffffff0d" : "#6366f10f") };
        return base;
    };

    return (
        <section id={(p.sectionId as string) || `block-${block.id}`} style={{ padding: editorPadding, background: bgColor, color: textColor, width: "100%", boxSizing: "border-box" }}>
            <div style={{ maxWidth: "100dvw", margin: "0 auto", boxSizing: "border-box", width: "100%" }}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: `repeat(${editorCols}, 1fr)`, gap: gap, marginTop: title || subtitle ? "4rem" : "0" }}>
